...
%
game_mode() :- retractall(choix(Choix)), write('Choix de l'),char_code(Guillemet, 39),write(Guillemet),write('heuristique :'), writeln('Random -> random.'), writeln('Duel -> duel.'), read(Choix), assert(choix(Choix)).
...
play(Player, Board) :- display_board(Player),choix(Choix), ((Choix='random',lis_random(Board, Player));(Choix='duel',lis(Board, Player))).
